...
// The expvar package is already based on a singleton for the different metrics that are registered with the package so there isn't much choice here.
var m *metrics
...
// The metrics value is stored in a package level variable since everything inside of expvar is registered as a singleton.
// The use of once will make sure this initialization only happens once.
func init() {
	m = &metrics{
...
// Add more of these functions when a metric needs to be collected in different parts of the codebase.
// This will keep this package the central authority for metrics and metrics won't get lost.
