...
func (evt *Events) Acquire(id string) chan string {
	...
	// Since a message will be dropped if the websocket receiver is not ready to receive,
	// this arbitrary buffer should give the receiver enough time to not lose a message.
	// Websocket send could take long.
...
// Send signals a message to ever registered channel.
// Send will not block waiting for a receiver on any given channel.
