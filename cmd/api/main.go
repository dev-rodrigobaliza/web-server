...
// appName will be used in several places, please set it accordingly your needs.
...
func run(log *zap.SugaredLogger) error {
	...
	// Configuration values will be passed through the application as individual values.
	cfg := struct {
		conf.Version
		Web struct {
	...
	// Parse will set the defaults and then look for any overriding values in environment variables and command line flags.
	help, err := conf.Parse(appName, &cfg)
	if err != nil {
	...
	// The handlers accept a function of this signature to allow the application to log.
	// For now, these raw messages are sent to any websocket client that is connected into the system through the events package.
	evts := events.New()
	// =========================================================================
	// To achieve this, uncomment the following lines and inject the evt object (func) into any object to generate websockets events
	// =========================================================================
	// evt := func(v string, args ...any) {
	...
	// The Debug function returns a mux to listen and serve on for all the debug related endpoints. This includes the standard library endpoints.
